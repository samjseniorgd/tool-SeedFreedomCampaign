let images = [];
let selectedIndices = [];
let scalePercent = 0.88; // Slightly reduced for taller format

// Dot grid variables - balanced for 4:5 ratio
const dotRows = 11;       // Balanced grid
const dotCols = 9;        // Balanced grid
const dotSize = 6;
...
function setup() {
  createCanvas(720, 900); // 4:5 aspect ratio
...
function calculateAreaDots() {
  ...
  // Define area boundaries - adjusted for 4:5
  const topHeight = height * 0.30;    // Standard 30%
  const bottomHeight = height * 0.70; 
  const leftWidth = width * 0.30;     
  const rightWidth = width * 0.70;    
