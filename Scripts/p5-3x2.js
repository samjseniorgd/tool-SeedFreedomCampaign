let images = [];
let selectedIndices = [];
let scalePercent = 0.9; // SET YOUR SCALE HERE

// Dot grid variables
const dotRows = 10;    // Number of rows
const dotCols = 10;    // Number of columns
...
function calculateAreaDots() {
  ...
  // Define area boundaries with 40% center gap
  const topHeight = height * 0.3;    // Top 30%
  const bottomHeight = height * 0.7; // Bottom 30% (from bottom)
  const leftWidth = width * 0.3;     // Left 30%
  const rightWidth = width * 0.7;    // Right 30% (from right)
