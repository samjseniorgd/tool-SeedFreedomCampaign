let images = [];
let selectedIndices = [];
let scalePercent = 0.92; // Increased for square format

// Dot grid variables - symmetric for square
const dotRows = 10;       // Same as original 3:2
const dotCols = 10;       // Same as original 3:2
const dotSize = 6;
...
function setup() {
  createCanvas(900, 900); // 1:1 aspect ratio
...
function calculateAreaDots() {
  ...
  // Define area boundaries - adjusted for square
  const topHeight = height * 0.32;    // Slightly increased
  const bottomHeight = height * 0.68; // Slightly increased gap
  const leftWidth = width * 0.32;     
  const rightWidth = width * 0.68;    
